...
import { DamageRelationOfPokemonType } from "../../types/DamageRelationOfPokemonType";
import {
  FlavorTextEntry,
  PokemonDescription,
...
const DetailPage = () => {
  ...
  const [isLoading, setIsLoading] = useState<boolean>(false); // 비동기작업 완료까지 표시할 로딩 state
  const params = useParams() as { id: string }; //id는 무조건 string이라고 단언 // path에 있는 포켓몬 id를 가져오기 위한 useParams
  ...
  async function fetchPokemonData(pokemonId: string) {
    ...
    try {
      const { data: pokemonData } = await axios.get<PokemonDetail>(url); // data의 이름을 pokemonData로 지정
      ...
      if (pokemonData) {
        ...
        const nextAndPreviousPokemon: NextAndPreviousPokemon =
          await getNextAndPreviousPokemon(id); // 이전, 다음 포켓몬데이터 가져오기
  ...
  const formatPokemonAbilities = (Abilities: Ability[]) =>
    ...
    Abilities.filter((_, index) => index <= 1).map(
      (
        obj: Ability // index가 1이하인 ability데이터만 가져오기
  ...
  async function getNextAndPreviousPokemon(id: number) {
    ...
    const nextResponse =
      pokemonData.next && (await axios.get<PokemonData>(pokemonData?.next)); // 다음 포켓몬데이터가 있다면 다음 포켓몬 정보 가져오기
    const previousResponse =
      pokemonData.previous &&
      (await axios.get<PokemonData>(pokemonData?.previous)); // 이전 포켓몬데이터가 있다면 이전 포켓몬 정보 가져오기
  ...
  const filterAndFormatDescriptions = (
    ...
    const koreanDescriptions = flavorText
      ?.filter((i: FlavorTextEntry) => i.language.name === "ko")
